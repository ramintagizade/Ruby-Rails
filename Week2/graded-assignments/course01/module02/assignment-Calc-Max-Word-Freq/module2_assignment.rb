...
class LineAnalyzer
  ...
  def initialize(text, number) 
    ...
    @content = text
    @line_number = number
    return self.calculate_word_frequency #call the method 
  end
  ...
  def calculate_word_frequency
    ...
    @content.split(" ").each do | word | 
      freq[word.downcase] = freq[word.downcase] + 1
    end
    freq.each do |word| 
      if word.last > @highest_wf_count 
        @highest_wf_count = word.last
      end
    end
    freq.each do |word| 
      if @highest_wf_count == word.last 
        @highest_wf_words.push(word.first)
      end
    end
    @highest_wf_words
  end
...
class Solution
  ...
  def analyze_file
    ...
    File.readlines(file).each do |line|
      @analyzers.push(LineAnalyzer.new(line,ids))
      ids = ids + 1
    end
  end
